...
Board.prototype = {
	...
	haveAllMinesBeenFlagged: function(){
		for (var i = 0; i < this.mines.lengnth; i++){
			...
			if ((this.map[mine[0]][mine[1]] & CELL_FLAG) != CELL_FLAG){
				return false;
			}
		}
	...
	revealMines: function(){
		...
		for (var x = 0;x < this.width;x ++){
			for (var y = 0;y <  this.height;y ++){
				if ((map[x][y] & CELL_MINE) == CELL_MINE)
					map[x][y] = map[x][y] | CELL_VISIBLE;				
			}
		}
	},

	floodFill: function(x, y, checked){
		if ((this.map[x][y] & CELL_MINE) == CELL_MINE)
			return;

		if (checked[x] && checked[x][y] == true)
			return;

		this.map[x][y] = this.map[x][y] | CELL_VISIBLE;
		...
		for (var x1 = Math.max(0, x - 1); x1 < Math.min(x + 1, this.width); x1++){
			for (var y1 = Math.max(0, y - 1); y1 < Math.min(y + 1, this.width); y1++){
				if ((this.map[x1][y1] & CELL_MINE) == CELL_MINE) return;
			}
		}
	...
	hitCell: function(x, y){
		...
		if ((this.map[x][y] & CELL_FLAG) == CELL_FLAG){
			this.map[x][y] = this.map[x][y] ^ CELL_FLAG;
		}

		if ((this.map[x][y] & CELL_EMPTY) == CELL_EMPTY) {
			this.floodFill(x, y, []);			
		}
	...
	getUnflaggedMineCount: function(){
		...
		for (var x = 0;x < this.width;x ++){
			for (var y = 0;y <  this.height;y ++){
				if ((this.map[x][y] & CELL_MINE) == CELL_MINE && 
					!((this.map[x][y] & CELL_FLAG) == CELL_FLAG)){
					count +=1;
				}				
			}
		}
	...
	flagCell: function(x, y){
		if ((this.map[x][y] & CELL_VISIBLE) == CELL_VISIBLE)
			return 0;
	...
	draw: function(containerEl){
		...
		for (var y = 0;y < this.height;y ++){
			...
			for (var x = 0;x <  this.width;x ++){
				...
				var isVisible = (map[x][y] & CELL_VISIBLE) == CELL_VISIBLE; 
				var isFlagged = (map[x][y] & CELL_FLAG) == CELL_FLAG; 
				if (isVisible && !isFlagged){
					if ((map[x][y] & CELL_MINE) == CELL_MINE)
					{
						cell.removeClass("flag empty fa fa-flag").addClass("mine fa fa-bomb");
					}				
					if ((map[x][y] & CELL_EMPTY) == CELL_EMPTY)
					{
						...
						for (var x1 = Math.max(0, x - 1); x1 <= Math.min(x + 1, this.width - 1); x1++){
							for (var y1 = Math.max(0, y - 1); y1 <= Math.min(y + 1, this.height - 1); y1++){
								if ((map[x1][y1] & CELL_MINE) == CELL_MINE) neighbors += 1;
							}
						}
	...
	getPossiblePoints: function(lastPoint, minX, minY, maxX, maxY){
		...
		for (var x = Math.max(minX, lastPoint[0] - oneOrZero()); x <= Math.min(maxX, lastPoint[0] + oneOrZero()); x++){
			for (var y = Math.max(minY, lastPoint[1] - oneOrZero()); y <= Math.min(maxY, lastPoint[1] + oneOrZero()); y++){
				if (x != lastPoint[0] && y != lastPoint[1] && this.map[x][y] === CELL_EMPTY){
					points.push([x, y]);
				}
			}
		}
	...
	traverseCluster: function(lastPoint, minX, minY, maxX, maxY, minesRemaining){
		...
		this.map[nextPoint[0]][nextPoint[1]] = CELL_MINE;
		this.mines.push(nextPoint);
	...
	buildCluster: function(border, clusterSize, minesPerCluster){
		...
		map[origin[0]][origin[1]] = CELL_MINE;	
		this.mines.push(origin);
